...
class _HeroSectionState extends State<HeroSection>
  ...
  void initState() {
    ...
    ).animate(CurvedAnimation(parent: _fadeCtrl, curve: AppMotion.easeOut));

    // One-shot entrance — respect reduced motion by jumping straight to end.
      ...
      child: Stack(
        children: [
          Positioned(
            top: -50,
            right: -50,
            child: _GlowOrb(size: 300, color: AppColors.primary.withValues(alpha: 0.07)),
          ),
          Positioned(
            bottom: 60,
            left: -60,
            child: _GlowOrb(size: 220, color: AppColors.secondary.withValues(alpha: 0.06)),
          ),
          Padding(
            padding: EdgeInsets.fromLTRB(28, isWide ? 120 : 132, 28, 48),
            child: Center(
                  ...
                  child: isWide
                      ...
                      : Column(
                          ...
                          children: [
                            ...
                            const Center(child: _DevIllustration()),
                          ],
                        ),
                ),
              ),
  ...
  Widget _buildLeft(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Desenvolvedor',
          style: textTheme.displayLarge?.copyWith(fontSize: 44, letterSpacing: -1.5),
        ),
        AnimatedSwitcher(
          ...
          transitionBuilder: (child, anim) => FadeTransition(
            ...
            child: SlideTransition(
              ...
              ).animate(CurvedAnimation(parent: anim, curve: AppMotion.easeOut)),
              child: child,
            ),
          ),
          child: ShaderMask(
        ...
        Row(
          ...
          children: [
            Flexible(
              child: Text(
                _typedText,
                style: textTheme.titleMedium?.copyWith(fontSize: 15, color: AppColors.text),
              ),
            ),
            AnimatedOpacity(
...
// Bespoke animation kept as-is (looping/decorative — no reduced-motion gate
// needed), colors restyled to AppColors tokens.
...
class _DevIllustrationState extends State<_DevIllustration>
  ...
  Widget build(BuildContext context) {
    final floatAnim = Tween<double>(begin: -7.0, end: 7.0).animate(
      CurvedAnimation(parent: _floatCtrl, curve: AppMotion.easeInOut),
    );
...
class _SkillsScreen extends StatelessWidget {
  ...
  static const _skills = [
    _SkillData('Flutter', 0.92, AppColors.primary),
    _SkillData('Dart', 0.88, AppColors.secondary),
    _SkillData('Firebase', 0.75, AppColors.success),
    _SkillData('REST API', 0.80, AppColors.primaryLight),
  ];
...
class _ProjectsScreen extends StatelessWidget {
  ...
  static const _projects = [
    _ProjectData('Flutter App', 'Mobile · Dart', Icons.phone_android_rounded, AppColors.primary),
    _ProjectData('Web Portal', 'Web · Flutter', Icons.web_rounded, AppColors.secondary),
    _ProjectData('Firebase API', 'Backend · Node', Icons.cloud_rounded, AppColors.success),
  ];
...
// ─── Floating Badge ───────────────────────────────────────────────────────────
// Kept as a lightweight custom widget rather than GlassCard: it's absolutely
// positioned via Positioned+Transform math tied to a fixed size, and
// GlassCard's shell padding would grow it and throw off that overlap.
